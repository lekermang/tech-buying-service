...
export default function RepairWidget() {
  const [step, setStep] = useState<"type" | "model" | "form">("type");
  ...
  const handleSubmit = async () => {
    ...
    const priceVal =
      totalPrice ??
      (simpleRepair ? Math.round((simpleRepair.priceFrom + simpleRepair.priceTo) / 2) : null);

    await fetch(REPAIR_ORDER_URL, {
      method: "POST",
    ...
    <div className="border border-white/10 bg-black/30 p-3 mb-3 w-full max-w-sm">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-oswald font-bold text-xs uppercase text-white tracking-wide">
            Ремонт телефонов
          </span>
          <span className="bg-[#FFD700] text-black font-oswald font-bold text-[10px] px-1.5 py-0.5 leading-none">
            От 20 мин!
          </span>
        </div>
        <Icon
      ...
      {open && (
        <div className="mt-3">
          {sent ? (
            <div className="flex items-center gap-1.5 text-[#FFD700] font-roboto text-xs border-t border-[#FFD700]/20 pt-2">
              <Icon name="CheckCircle" size={13} />
              Заявка принята! Перезвоним через 15 мин.
              <button onClick={reset} className="ml-auto text-white/40 hover:text-white text-[10px]">
                Ещё раз
              </button>
            </div>
          ) : step === "type" ? (
            <div className="border-t border-white/10 pt-2">
              <div className="font-roboto text-white/40 text-[10px] mb-2 uppercase tracking-wide">
              ...
              <div className="grid grid-cols-1 gap-1">
                {[
                  ...
                  <button
                    key={t.id}
                    onClick={() => {
                      setRepairType(t.id as typeof repairType);
                      setStep("model");
                    }}
                  ...
                  >
                    ...
                    {t.label}
                    {(t.id === "display" || t.id === "battery") && (
                      <span className="ml-auto text-white/30 text-[10px]">цена по каталогу</span>
                    )}
                    {t.id === "simple" && (
                      <span className="ml-auto text-white/30 text-[10px]">600–2000 ₽</span>
                    )}
                  </button>
                ))}
              </div>
            ...
            <div className="border-t border-white/10 pt-2">
              <button
                ...
                className="flex items-center gap-1 text-white/40 hover:text-white font-roboto text-[10px] mb-2 transition-colors"
              >
              ...
              />
              {partsLoading && (
                <div className="text-white/30 font-roboto text-[10px] py-1">Загружаю каталог...</div>
              )}
              {modelQuery.length > 1 && !partsLoading && (
                <div className="max-h-32 overflow-y-auto border border-white/10 mb-2">
                  {filtered.length === 0 ? (
                    <div className="px-3 py-2 font-roboto text-white/40 text-[10px]">
                      Не найдено — оставьте заявку, уточним цену
                    </div>
                  ) : (
                    filtered.slice(0, 20).map((p, i) => (
                      <button
                        ...
                        onClick={() => {
                          setSelectedPart(p);
                          setModelQuery(normalizeModel(p.model));
                        }}
                      ...
                      >
                        ...
                        <span className="text-[#FFD700] font-bold">
                          {(p.price + REPAIR_COST).toLocaleString("ru-RU")} ₽
                        </span>
                      </button>
                    ))
                  )}
                ...
                <div className="bg-[#FFD700]/5 border border-[#FFD700]/20 px-3 py-2 mb-2">
                  ...
                  <div className="flex justify-between items-baseline">
                    <span className="font-roboto text-[10px] text-white/50">
                      Запчасть {selectedPart.price.toLocaleString("ru-RU")} ₽ + работа {REPAIR_COST} ₽
                    </span>
                    <span className="font-oswald font-bold text-[#FFD700] text-base">
              ...
              {simpleRepair && (
                <div className="bg-[#FFD700]/5 border border-[#FFD700]/20 px-3 py-2 mb-2">
                  ...
                  <div className="font-oswald font-bold text-[#FFD700] text-sm">
                    {simpleRepair.priceFrom}–{simpleRepair.priceTo} ₽
                  </div>
                </div>
              )}
