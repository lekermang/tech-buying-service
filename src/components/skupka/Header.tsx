  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50">
      ...
      <div className="bg-[#0D0D0D]/95 backdrop-blur-sm border-b border-[#FFD700]/20">
          ...
          <nav className="hidden md:flex lg:hidden items-center gap-3">
            {NAV_LINKS.slice(0, 3).map(l => (
              <button key={l.href} onClick={() => handleNav(l.href)}
                className="font-roboto text-xs text-white/60 hover:text-[#FFD700] transition-colors uppercase tracking-wide">
                {l.label}
              </button>
            ))}
            <a href="/catalog" className="font-roboto text-xs text-white/60 hover:text-[#FFD700] transition-colors uppercase tracking-wide flex items-center gap-1">
        ...
        {menuOpen && (
          <div className="md:hidden fixed inset-0 top-[88px] bg-[#0D0D0D] z-50 flex flex-col overflow-y-auto">
            ...
            <div className="px-4 pt-4 pb-6 mt-auto space-y-3">
