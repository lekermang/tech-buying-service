...
const ToolsProductCard = memo(function ToolsProductCard({ p, onAdd }: Props) {
  const d = disc(p);
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-lg overflow-hidden flex flex-col hover:shadow-lg hover:shadow-black/40 hover:border-orange-500/50 transition-all group cursor-pointer">
      <div className="relative bg-gray-800 flex items-center justify-center overflow-hidden" style={{ aspectRatio: "1" }}>
        ...
        {p.image_url && !imgErr ? (
          <img
            ...
            loading="lazy"
            decoding="async"
            className={`w-full h-full object-contain p-3 group-hover:scale-105 transition-all duration-300 ${imgLoaded ? "opacity-100" : "opacity-0"}`}
      ...
      <div className="p-3 flex flex-col flex-1">
        <div className="text-[10px] text-gray-700 font-mono mb-1 truncate">Арт. {p.article}</div>
        ...
        <div className="flex items-center gap-1.5 mb-2.5">
          ...
          <span className={`text-[11px] ${inStock ? "text-green-400" : "text-gray-600"}`}>
            {inStock ? "В наличии" : p.amount === "Под заказ" ? "Под заказ" : "Уточняйте"}
          </span>
        </div>
        ...
        <div className="mb-3">
          {myPrice > 0 ? (
            <>
              ...
              <div className="text-lg font-bold text-white">{fmt(myPrice)}</div>
            </>
          ) : (
        ...
        <button
          ...
          className={`w-full py-2 text-sm font-semibold rounded transition-all ${
            added ? "bg-green-600 text-white" : "bg-orange-500 hover:bg-orange-600 text-white"
          }`}
        >
...
export default ToolsProductCard;
