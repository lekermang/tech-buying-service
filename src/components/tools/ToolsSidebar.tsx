...
export default function ToolsSidebar({
  ...
  return (
    <>
      ...
      `}>
        <div className="lg:bg-gray-900 lg:rounded-lg lg:border lg:border-gray-800 p-4">

          {/* Мобильный заголовок */}
          <div className="flex items-center justify-between mb-4 lg:hidden">
            <span className="font-bold text-white text-lg">Фильтры</span>
            <button onClick={() => setSidebarOpen(false)} className="text-gray-500 hover:text-gray-300"><Icon name="X" size={20} /></button>
          </div>

          {activeFilters > 0 && (
            <button onClick={clearAll} className="flex items-center gap-1.5 text-sm text-orange-400 hover:text-orange-300 mb-4 transition-colors font-medium">
              <Icon name="X" size={13} />Сбросить все фильтры
            </button>
          )}

          {/* В наличии */}
          <label className="flex items-center gap-3 cursor-pointer mb-5 pb-5 border-b border-gray-800"
            onClick={() => handleStock(!inStockOnly)}>
            <div className={`w-10 h-5 rounded-full transition-colors relative shrink-0 ${inStockOnly ? "bg-orange-500" : "bg-gray-700"}`}>
            ...
            <span className="text-sm text-gray-300 font-medium">Только в наличии</span>
          </label>
