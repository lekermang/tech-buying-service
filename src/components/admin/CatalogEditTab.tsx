...
export default function CatalogEditTab({ token }: { token: string }) {
  ...
  const handleBulkMarkup = async (wholesaleDelta: number, retailDelta: number) => {
    ...
    try {
      ...
      const d = await res.json();
      if (d.ok) { setMarkupResult(d); load(search, filterCat, filterAvail, page); }
  ...
  const handleSendEmail = async () => {
    ...
    const PRICE_URL = "https://functions.poehali.dev/b09271ea-c662-4225-973f-4dd4c6a0e32c";
    try {
      ...
      <div className="mb-4 border border-[#2a2a2a] bg-[#111]">
        ...
        <div className="p-3 flex flex-wrap gap-4 items-end">
          ...
          <div>
            ...
            {markupApplying && <div className="font-roboto text-[10px] text-[#FFD700]/60 mt-1 flex items-center gap-1"><Icon name="Loader" size={10} className="animate-spin"/>Применяю...</div>}
            {markupResult && (
