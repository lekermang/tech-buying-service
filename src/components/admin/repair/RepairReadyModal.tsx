...
import { Order, EditForm, inp, lbl } from "./repairTypes";
...
export default function RepairReadyModal({ order, form, error, saving, onFormChange, onSubmit, onClose }: Props) {
  return (
      ...
      <div className="bg-[#1A1A1A] border border-[#FFD700]/40 w-full max-w-sm p-5" onClick={e => e.stopPropagation()}>
        ...
        <div className="space-y-3">
          <div>
            ...
            <input value={form.parts_name} onChange={e => onFormChange({ ...form, parts_name: e.target.value })}
              placeholder="Дисплей iPhone 14, аккумулятор..." className={inp} />
          </div>
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className={lbl + " text-orange-400/80"}>💸 Закупка (₽) *</label>
              <input type="number" value={form.purchase_amount} onChange={e => onFormChange({ ...form, purchase_amount: e.target.value })}
                placeholder="500" className={inp} />
            </div>
            <div>
              <label className={lbl + " text-green-400/80"}>💰 Выдано за ремонт (₽) *</label>
              <input type="number" value={form.repair_amount} onChange={e => onFormChange({ ...form, repair_amount: e.target.value })}
          ...
          <div>
            ...
            <textarea value={form.admin_note} onChange={e => onFormChange({ ...form, admin_note: e.target.value })}
        ...
        <div className="flex gap-2 mt-4">
          <button onClick={onSubmit} disabled={saving}
            ...
            <Icon name="Check" size={15} />{saving ? "Сохраняю..." : "Подтвердить «Готово»"}
          </button>
          <button onClick={onClose} className="px-4 text-white/30 font-roboto text-xs hover:text-white transition-colors">Отмена</button>
        </div>
      </div>
    </div>
  );
