...
function ItemDetail({ token, item, onClose, onUpdated, onSell }: { token: string; item: SLItem; onClose: () => void; onUpdated: () => void; onSell: () => void }) {
  ...
  const save = async () => {
    const r = await slApi(token, "item_update", { method: "POST", body: { id: item.id, ...data } });
    if (r.ok) onUpdated();
  };
          ...
          {!editing ? (
            ...
            <>
              ...
              <div className="flex gap-2">
                <button onClick={() => setEditing(false)} className="flex-1 bg-[#141414] py-2 rounded-lg text-sm">Отмена</button>
                <button onClick={save} className="flex-1 bg-[#FFD700] text-black font-bold py-2 rounded-lg text-sm">Сохранить</button>
              </div>
            </>
          )}
        </div>
