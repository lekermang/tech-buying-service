...
import {
  ACCENT, ICONS, AUTHENTICITY, PROCESS, WHY_US,
  ICON_PRICES, ICON_STORIES, ICON_FAQ,
} from "./data";
...
const IconsContent = ({ activeTab, setActiveTab, activeFaq, setActiveFaq }: IconsContentProps) => {
    ...
    <>
      ...
      <section id="icons" className="max-w-6xl mx-auto px-4 py-14 md:py-20">
        <div className="mb-8">
          ...
          <p className="font-roboto text-white/45 text-sm mt-1">Цены ориентировочные — итоговая стоимость зависит от сохранности, школы и наличия оклада</p>
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/[0.07] bg-[#0D0D0D]">
                <th className="text-left font-roboto text-[11px] uppercase tracking-widest text-white/30 px-5 py-3">Икона / тип</th>
                ...
                >
                  <td className="px-5 py-4">
                    <div
                      ...
                      onMouseLeave={(e) => ((e.currentTarget as HTMLDivElement).style.color = "white")}
                    >
                      {item.name}
                    </div>
                  ...
                  <td className="px-5 py-4 hidden sm:table-cell">
                    <span className="font-roboto text-xs px-2 py-0.5 rounded-full border" style={{ color: activeCategory.color, borderColor: `${activeCategory.color}40`, background: `${activeCategory.color}15` }}>
                      {item.period}
                    </span>
                  </td>
        ...
        <p className="font-roboto text-white/25 text-xs mt-3 text-right">* Итоговая цена определяется после осмотра иконы экспертом-реставратором</p>
      </section>
      ...
      <section className="border-y border-white/5 py-14 md:py-20" style={{ background: "#0A0A0A" }}>
        <div className="max-w-6xl mx-auto px-4">
          <div className="mb-10">
            ...
            <h2 className="font-oswald font-bold text-3xl md:text-4xl uppercase">Как определить<br /><span style={{ color: ACCENT }}>ценность иконы</span></h2>
          </div>
      ...
      <section className="bg-[#0A0A0A] border-y border-white/5 py-14 md:py-20">
        <div className="max-w-6xl mx-auto px-4">
          <div className="mb-10">
            ...
            <h2 className="font-oswald font-bold text-3xl md:text-4xl uppercase">Сколько стоит <span style={{ color: ACCENT }}>ваша икона</span></h2>
            <p className="font-roboto text-white/45 text-sm mt-1">Ориентировочные цены — итоговая стоимость зависит от состояния и провенанса</p>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {ICON_PRICES.map((p, i) => (
              <div key={i} className="bg-[#0D0D0D] border border-white/[0.06] rounded-xl overflow-hidden flex flex-col" style={{ borderTopColor: p.color, borderTopWidth: 2 }}>
                <div className="p-5 flex flex-col gap-3 flex-1">
                  <div>
                    <div className="font-oswald font-bold text-base leading-tight mb-0.5">{p.school}</div>
                    <div className="font-roboto text-xs" style={{ color: `${p.color}99` }}>{p.period}</div>
                  </div>
                  <div className="flex items-baseline gap-2">
                    <span className="font-oswald font-black text-2xl leading-none" style={{ color: p.color }}>{p.min}</span>
                    <span className="font-roboto text-white/30 text-xs">—</span>
                    <span className="font-oswald font-bold text-lg leading-none" style={{ color: `${p.color}bb` }}>{p.max}</span>
                  </div>
                  <ul className="flex flex-col gap-1 mt-auto">
                    {p.examples.map((ex, j) => (
                      <li key={j} className="flex items-start gap-1.5">
                        ...
                        <span className="font-roboto text-white/45 text-xs leading-relaxed">{ex}</span>
                      </li>
                    ))}
                  </ul>
                </div>
          ...
          <div className="grid md:grid-cols-3 gap-5">
            {ICON_STORIES.map((s, i) => (
              <div key={i} className="bg-[#0D0D0D] border border-white/[0.06] p-6 rounded-xl flex flex-col gap-4">
                ...
                <div>
                  <div className="font-oswald font-black text-3xl" style={{ color: ACCENT }}>{s.result}</div>
