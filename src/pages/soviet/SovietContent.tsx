...
import {
  ACCENT, ANTIQUES, AUTHENTICITY, PROCESS, WHY_US,
  SOVIET_PRICES, SOVIET_STORIES, SOVIET_FAQ,
} from "./data";

interface SovietContentProps {
  activeTab: number;
...
const SovietContent = ({ activeTab, setActiveTab, activeFaq, setActiveFaq }: SovietContentProps) => {
    ...
    <>
      ...
      <section id="soviet" className="max-w-6xl mx-auto px-4 py-14 md:py-20">
        <div className="mb-8">
          ...
          <p className="font-roboto text-white/45 text-sm mt-1">Цены ориентировочные — итоговая стоимость зависит от редкости, периода и сохранности</p>
        </div>
              ...
              {activeCategory.items.map((item, idx) => (
                ...
                >
                  <td className="px-5 py-4">
                    <div className="font-oswald font-bold text-sm text-white">{item.name}</div>
      ...
      <section className="border-y border-white/5 py-14 md:py-20" style={{ background: "#0A0A0A" }}>
        <div className="max-w-6xl mx-auto px-4">
          <div className="mb-10">
            ...
            <h2 className="font-oswald font-bold text-3xl md:text-4xl uppercase">Как определить<br /><span style={{ color: ACCENT }}>ценность предмета</span></h2>
          </div>
      ...
      <section className="max-w-6xl mx-auto px-4 py-14 md:py-20">
        <div className="mb-10">
          ...
          <h2 className="font-oswald font-bold text-3xl md:text-4xl uppercase">Что и сколько <span style={{ color: ACCENT }}>стоит</span></h2>
        </div>
        <div className="grid md:grid-cols-2 gap-5">
          {SOVIET_PRICES.map((cat, i) => (
            <div key={i} className="bg-[#0D0D0D] border border-white/[0.06] rounded-xl overflow-hidden" style={{ borderTopColor: cat.color, borderTopWidth: 2 }}>
              <div className="px-5 pt-5 pb-4 flex items-center gap-2 border-b border-white/[0.05]">
                <span className="text-xl leading-none">{cat.icon}</span>
                <span className="font-oswald font-bold text-base" style={{ color: cat.color }}>{cat.category}</span>
              </div>
              <div className="p-5 flex flex-col gap-2.5">
                {cat.items.map((item, j) => (
                  <div key={j} className="flex items-start justify-between gap-3">
                    <span className="font-roboto text-white/60 text-sm leading-relaxed">{item.name}</span>
                    <div className="flex items-center gap-2 shrink-0">
                      {item.hot && (
                        <span className="font-roboto text-[10px] px-1.5 py-0.5 rounded-full whitespace-nowrap" style={{ background: `${ACCENT}20`, color: ACCENT, border: `1px solid ${ACCENT}40` }}>
                          🔥 Горячий спрос
                        </span>
                      )}
                      <span className="font-oswald font-bold text-sm whitespace-nowrap" style={{ color: cat.color }}>{item.price}</span>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <section className="py-14 md:py-20 border-t" style={{ borderColor: `${ACCENT}15`, background: "#0A0A0A" }}>
        <div className="max-w-6xl mx-auto px-4">
          <div className="mb-10">
            ...
            <h2 className="font-oswald font-bold text-3xl md:text-4xl uppercase">Реальные находки <span style={{ color: ACCENT }}>наших клиентов</span></h2>
          </div>
