...
def find_similar_buys(text: str, limit: int = 6) -> list:
    """Возвращает похожие закупки: model, storage, condition, buy_price, sell_price."""
    kw = _keywords(text)
    if not kw:
        return []
    ...
    if not dsn:
        return []
    # Ключевые токены: бренд/модель/числа. Берём самые значимые (бренд + модель + объём).
...
def buys_hint(text: str) -> str:
    """Готовый текстовый блок с реальными закупками для подмешивания в промпт ИИ."""
